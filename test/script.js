...
const notifications = [
  { title: "テスト通知", body: "これはデフォルトのテスト通知です。" },
  { title: "情報通知", body: "これは情報タイプの通知です。" },
  { title: "警告通知", body: "これは警告タイプの通知です。" },
  { title: "エラー通知", body: "これはエラータイプの通知です。" },
];

let currentIndex = 0;

function updateButton(enabled) {
  sendNotificationBtn.disabled = !enabled;
}
...
sendNotificationBtn.addEventListener("click", () => {
  ...
  const notify = notifications[currentIndex];
  new Notification(notify.title, {
    body: notify.body,
    // icon: "/icon.svg" // 必要ならここにアイコンパス
  });
